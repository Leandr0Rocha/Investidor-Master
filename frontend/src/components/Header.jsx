...
const Header = ({ children }) => (
  <header className="flex flex-col lg:flex-row items-center justify-between pt-8 max-w-6xl mx-auto flex-1 min-h-screen relative overflow-hidden px-2 sm:px-4">
    <div className="flex flex-col gap-0">
      <div className="w-full md:w-3/4 text-center md:text-left flex flex-col items-center md:items-start justify-center h-full gap-0 z-10">
        <h1 className="text-xl xs:text-3xl sm:text-4xl md:text-5xl font-extrabold text-amareloClaro mb-2 font-title leading-tight drop-shadow-lg">
      ...
      <div className="w-full md:w-1/2 flex flex-col items-start justify-center h-full gap-0 z-10 text-left mt-2 md:mt-0">
        {children}
      </div>
    </div>
