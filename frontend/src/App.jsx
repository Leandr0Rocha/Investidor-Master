...
function App() {
  return (
    <div className="min-h-screen bg-gradient-to-b from-black via-[#BE7B0E]/60 to-black flex flex-col overflow-x-hidden">
      <header className="flex flex-col md:flex-row items-center justify-between pt-8 max-w-6xl mx-auto flex-1 min-h-screen relative overflow-hidden px-2 sm:px-4">
        <div className="w-full md:w-1/2 text-center md:text-left flex flex-col items-center md:items-start justify-center h-full gap-0 z-10">
          <h1 className="text-2xl xs:text-3xl sm:text-4xl md:text-5xl font-extrabold text-text-general mb-1 font-title leading-tight drop-shadow-lg">
            Investidor master com <br className="hidden md:block" />
            <span className="text-button-name">Carlos Goodman</span>
          </h1>
          <p className="text-text-general font-semibold mb-1 text-sm xs:text-base sm:text-lg md:text-xl drop-shadow">
            Aprenda a investir em 4 aulas ao vivo com a expertise e experiência do Carlos Goodman
          </p>
          <p className="text-text-general mb-2 text-xs xs:text-sm sm:text-base md:text-lg leading-relaxed">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Phasellus felis metus, rutrum id mollis ac, lobortis sit amet libero. Integer consequat nunc velit interdum efficitur.
          </p>
          <div className="flex flex-col gap-4 w-full max-w-xs xs:max-w-sm sm:max-w-md mt-6">
            <LeadForm />
          </div>
        </div>
